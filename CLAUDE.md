...
- **구현 완료 PR 머지 시**: TASKS 의 subitem/부모를 완료 표기(`[x]`)하고 모든 subitem 완료 시 Done 으로 옮긴다. **★설계문서(`doc/NN`)에는 'dev 구현 체크리스트'를 두지 않는다(D71).** 구현 상태·PR 완료·테스트 건수 등 **작업 히스토리는 `doc/TASKS.md`·`doc/PROJECT_LOG.md` 로 일원화**한다. 설계문서는 **설계·근거·현재 동작**만 담는다(구 D28 의 '설계문서 dev 체크리스트 동기 갱신' 관행은 D71 로 폐지).
