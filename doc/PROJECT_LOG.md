...
- **처리율 튜닝(사용자, env-only 순차)**: tick-interval PT5M→**PT2M**(묶음 간격 7~8분→2~3분), domains-per-tick 100→**500**(전수 재순회 37h→~7.5h). 둘 다 delta-driven(값싼 skip)+max-queries-per-hour 캡(3000, 주야 공통·off-peak 상향 없음)으로 Loki 보호. 관측: skip 61%·scan failed 0. drift 는 아직(시간/추가 throughput 또는 skip→now 즉시전진 최적화 필요). ★야간 몰아치기 방지 확인: off-peak 는 K·window 만 스위치, 쿼리캡 미상향 + off-peak-max-window PT1H·off-peak-domains-per-tick 300(<주간 500).
