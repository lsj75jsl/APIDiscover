...
> deprecated 미표기라도 "신버전 active + 구버전 트래픽" 이면 구버전을 **추정 Zombie** 로 재분류하고, 모든 Zombie 에 조치 시급성 **severity** 를 산정한다. 근거 [04-matching-and-classification](04-matching-and-classification.md) §5(버전 보강)·§4.2(severity), 결정 [DECISIONS](DECISIONS.md) **D23**.
> 연계: [24-cross-scan-recency-zombie-severity](24-cross-scan-recency-zombie-severity.md)(cross-scan recency 보너스), [25-report-output-enhancements](25-report-output-enhancements.md) §B(Zombie params).

**구현 위치**

| 대상 | 소스 |
|---|---|
| severity 점수·밴드 | `model/Severity(score)` + `band()` / `model/SeverityBand{LOW,MEDIUM,HIGH}` |
| severity 산정 | `classify/ZombieSeverity.of(Evidence)`(base) / `of(Evidence, historicalFirstSeen)`(+recency, doc/24) |
| 버전 추정 | `classify/VersionZombieInference`(`^v(\d+)$` 페어링) + `Evidence` |
| 오케스트레이션 | `classify/Classifier`(1st pass `Map<String,Evidence>` 누적 → 2nd pass 추정+severity) |
| Zombie finding | `model/Finding.Zombie`(confidence·severity·estimated·params) |
...
```mermaid
flowchart TD
    A["active(observed & 비-deprecated) endpoint"] --> V{"세그먼트에 ^v(\d+)$?"}
    V -->|"아니오"| SKIP["비버전 → 추정 대상 외"]
    V -->|"예"| K["resourceKey = method｜host｜버전위치 {V} 치환 template"]
    K --> G["같은 resourceKey 그룹핑 → Vmax 산출"]
    G --> C{"version < Vmax?"}
    C -->|"예"| Z["추정 Zombie (confidence 0.6, estimated=true)"]
    C -->|"아니오 (=Vmax)"| KEEP["Active 유지"]
```
...
- `spanScore = clamp(log10((lastSeen−firstSeen)초 + 1)/4, 0..1)` — window 내 **지속 사용**.
  (단일 스캔 window 내 recency 대용: 관측 자체가 "최근 window" 보장 + 지속될수록 고착.)
- `score = 0.5·hitsScore + 0.3·successScore + 0.2·spanScore` → band: ≥0.66 HIGH / ≥0.33 MEDIUM / else LOW. (가중치·임계 1차값, 튜닝 가능.)
- 위 `score` 는 **base**. 이후 [24](24-cross-scan-recency-zombie-severity.md) 가 cross-scan 누적 lifespan 기반 **entrenchment 보너스**를 더한다(`ZombieSeverity.of(Evidence, historicalFirstSeen)`). 단일 스캔 경로(`of(Evidence)`)는 보너스 0 → base 와 동일(무회귀).
- **모든 Zombie 에 적용**(명시 deprecated + 추정). 매칭 Evidence(누적 hits/firstSeen/lastSeen/2xx/total)에서 산출.
- **confidence vs severity 역할 분리**(doc/04 §4.2): confidence="진짜 Zombie 인가"(명시 1.0/추정 0.6), severity="조치 시급성"(트래픽 메트릭). 서로 독립적(별개 축).

## 3. Finding.Zombie shape + 노출

- `Finding.Zombie(host, method, pathTemplate, double confidence, Severity severity, boolean estimated, String specRef, String reason, ParamCandidates params)` — **severity·estimated 가산**(이후 `params` 도 추가, doc/25 §B).
...
- ✅ cross-scan recency — 스캔 히스토리 기반 entrenchment 보너스로 구현됨([24](24-cross-scan-recency-zombie-severity.md)). base severity 에 가산(단일 스캔 무회귀).
- **남은 한계**: 추정 임계·severity 가중치 중앙 API 설정(필요 시 yml→중앙, seam=`@ConfigurationProperties`) — 후속.
