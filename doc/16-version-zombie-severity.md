...
### 신규
- [x] `model/Severity(double score)` + `@JsonProperty("band") band()` 파생 + `enum SeverityBand{LOW,MEDIUM,HIGH}`.
- [x] `classify/ZombieSeverity` — `of(Evidence)→Severity`(hitsScore·successScore·spanScore 가중합→band). 순수·결정적.
- [x] `classify/VersionZombieInference` — `^v\d+$` 버전 식별·resourceKey 페어링·추정 규칙 → 추정 Zombie 키 집합.
      + `Evidence`(누적 hits/firstSeen/lastSeen/2xx/total) 소형 타입.

### 수정
- [x] `model/Finding.Zombie` — `Severity severity` + `boolean estimated` 필드 추가(가산).
- [x] `classify/Classifier` — `observedSpecKeys: Set→Map<String,Evidence>`(1st pass 메트릭 누적), 2nd pass 버전 추정+severity 배선
      (명시 1.0/추정 0.6, 모든 Zombie severity). classify/classifyWithMetrics 반영.

### 테스트
- [x] `VersionZombieInferenceTest` — v1+v2 observed·v1 비deprecated→v1 추정; 단일/동일 버전 무; 비버전 무; 신버전 Unused→구버전 비Zombie;
      명시 deprecated 구버전 중복 없음; 다중 구버전; 버전 위치 인식.
- [x] `ZombieSeverityTest` — band 경계(hits/2xx/span 버킷), HIGH vs LOW, total=0·firstSeen=lastSeen 엣지.
- [x] `Classifier` 통합 — 명시 deprecated Zombie 에 severity+estimated=false·**confidence 1.0 무회귀**; 추정 Zombie 0.6·estimated=true;
      비버전 spec Active 유지(무회귀); evidence 합산(host-agnostic 다중 host).
- [x] 하위호환 — 기존 Zombie/Active/Unused(비버전) 동작 동일, ClassifierTest 생성자 갱신, 리포트 가산 필드.
