...
- **검증**: 실 PG 회귀가드 3건 — `specMetaEndpointsDoNotMaterializeRawDocOidInAutoCommit`(M2/M4/M6 oid) + `forHostEndpointsTolerateRawDocOidSpecOnRealPg`(/discovery·/result oid) + `specListNullSpecNameOrdersFirstDeterministicallyOnRealPg`(M6 nulls-first 정렬). 각각 fix 임시 원복(projection→엔티티 / @Transactional 제거 / nulls first 제거) 시 RED(정확한 oid 에러 / PG NULLS LAST 순서 발산) 확인 후 GREEN(H2 재현 불가).
- **후속(별도)**: rawDoc `@Lob byte[]` oid→bytea 매핑 정밀화(마이그레이션 위험) — 실수요 시.

### D52. M7 — spec 멀티문서 관리 + API 상태추적 (ADDED/DELETED/UPDATED) (doc/36) — 제안·권장안
