...
> doc/37 §10 P2 상세화. 근거 결정 **DECISIONS D54**. **P1 머지(main `3b70c3e`) 위에 설계**. 근거=실제 머지 코드 확인(`ApiInventoryService`·`DocumentedApiRecord`·`SpecStore.loadActiveCanonical`·`SpecCanonicalizer.merge`·`Classifier` DELETED 결합). 운영 Loki 미호출(정적).
> ★설계만 — 구현 금지(설계·문서·TASKS 까지). 매니저가 (P2-1 위험 시) 사용자 확인 후 dev.
> **묶음 권고 = P2-2 + P2-3 + P2-4 를 1 PR**. ★**P2-1(매칭 source 대체) = 보류 권고**(이 PR 제외, §1). **P2-5(inactive prune) = 범위 밖**(TODO 유지).
...
### 3.5 노출
- `/apis` 응답(`DocumentedApiView`)에 `lastChangeBreaking`(boolean)·`changedParams`(= lastChangeDetailJson 파싱) **가산**. UPDATED 행에서 의미. 다른 lastChange 는 false/null.
- 필터 `?breaking=true`(선택) — breaking UPDATED 만. additive.

## 4. P2-4 도메인-merged 뷰

### 4.1 노출 — `?view=merged`(신규 엔드포인트 대신)
현 `/apis`=문서(specName)별 행. 도메인 전체 표면을 문서 구분 없이 병합·dedupe(키 `method+path_template`)한 뷰를 **기존 `/apis` 의 `?view=merged`** 로 제공(신규 엔드포인트보다 surface 적음·권고). 기본(`?view` 무)=현행 per-document 목록(무회귀).
...
### 4.3 응답 shape (샘플)
```jsonc
// GET /api/v1/domains/{host}/apis?view=merged
[
...
- **중앙 영향**. `/apis` 에 `lastChangeBreaking`·`changedParams` 필드 + `?view=merged`·`?breaking=true` = **전부 additive**(기존 소비자 무영향·신규 필드/옵션). **매칭 내부 변경 없음**(P2-1 보류 → 분류 외부 동작 불변). 신규 컬럼=워커 로컬 DB(중앙 무관).

## 6. 실 PG 테스트 계획 (Testcontainers podman)

- **P2-3 breaking 규칙**(각 케이스 재업로드 → `/apis` 단언). required 추가=breaking·optional 추가=non-breaking·optional 제거=breaking·optional→required=breaking·required→optional=non-breaking·type 비호환=breaking. `lastChangeBreaking` + `changedParams`(added/removed/modified) 영속·노출 확인.
...
- **이 PR(P2 묶음) = P2-2 + P2-3 + P2-4**. dev 체크리스트 → TASKS subitem(D26·부모 P 버킷).
  ...
  - P2-3: `applyUpdate` delta+breaking 계산 + 컬럼 2개 + `DocumentedApiView` 가산 + `?breaking` + 파서 type 호환표 + 실 PG 테스트.
  - P2-4: `ApiInventoryService` merged 산출(병합 규칙) + `?view=merged` + 응답 + 실 PG 테스트.
- **★P2-1 = 보류**(이 PR 제외). 추진 결정 시 별도 PR + §1.6 안전경로(MERGE/VERSION_GROUPED 한정·폴백·골든 이중경로) 선행. 매니저가 사용자에 **위험/가치(동작보존=가시가치 0·분류 직격 위험)** 전달 후 결정.
- **P2-5 inactive prune = 범위 밖**(TODO 유지) — `spec_record` inactive 누적은 희소·무해, 상한 prune 은 수요 시.

## 8. 무회귀 / 리스크 (정직)

- **무회귀** — P2-3 컬럼 2개 ADD(기존행 false/null)·`/apis` 필드/옵션 가산·P2-4 compute-on-read·매칭/분류 경로 불변(P2-1 보류). reconcile 의 기존 거동(삭제 격리·UPDATED)은 유지하고 breaking 계산만 **가산**.
