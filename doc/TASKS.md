...
- [x] **신규 도메인 즉시 처리 CLI — `-domain -register` 신규 + `-domain -scan` 미등록 자동등록** **(구현 완료 — build green 445, 커밋 보류·머지 시 Done. doc/31 B1·32 §4·33 §7·DECISIONS D47 확장, 브랜치 feat/cli-register-and-scan-autoregister)** — 운영자가 신규 도메인을 스케줄러 안 기다리고 즉시 등록/스캔. 등록 로직은 공유 헬퍼 `DomainRegistrar.registerIfAbsent`(register·scan 자동등록 재사용, REST `DomainController` 는 409 라 CLI 부적합·자동 디스커버리 `DomainUpserter` 는 `discoveredAt` 마킹이라 부적합 → 별도 최소 컴포넌트). 운영 Loki 미호출(register=DB only, scan 만 기존대로 Loki).
  ...
  - [x] 문서 동기 — doc/31 B1·32 §4(register 예시+scan 자동등록 주석)·33 §7·DECISIONS D47 확장. HTML 매뉴얼=technical_writer 후속(미터치).
