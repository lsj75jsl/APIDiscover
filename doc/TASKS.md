...
- [ ] **무접속 도메인 상태 관리 — 7일 무요청 비활성·경로 제외·재활성 (구현·배포 완료 doc/43·D82·main `5bced89`·이미지 `4d876686b814` .197 배포, 2026-07-20. 잔여=사후 관찰만)** — 원인: 워터마크 롱테일 발산은 window 상한·라운드로빈 기아 아닌 inactive-after=P3D 게이트 스킵(굶는 enabled 8,201=100% lastSeenAt>3d). **사용자 확정: 임계 P7D·status 안B 영속 enum·수동 스캔 승격.** 566 테스트 green·롤백 `prev-d82`(a171edf).
  ...
  - [x] **scan 정합** (main `c07891e`·이미지 `7632f074c9f0` 배포·롤백 `prev-scanfix`) — `DiscoveryJobService.analyze()` 가 `discovery.excluded-paths` 요청을 인벤토리에서도 제외(discovery 와 일관·rawPath 접두 매칭). 신규 discovered_endpoint 에 `.cloudbric` 미기록. 569 테스트 green. ★기존 잔존 5,430행(5,037 host)은 단일테이블 DELETE 로 1회 정리 필요(FK 없음·안전) — 사용자 승인 대기.
  ...
  - [ ] 후속(대기): `.cloudbric` 기존 endpoint 5,430행 정리(승인 시) / 3단계 endpoint 0 드릴다운 / full eTLD+1(PSL·SaaS 카운팅).
