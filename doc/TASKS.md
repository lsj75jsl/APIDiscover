...
- [ ] **REST API 대규모 변경 배치 (삭제·수정·신규)** **(설계 → doc/35 / D50. PR1(D1+M1+M3) #39·PR2(filename+M2+M4+M6) #40·PR3(M5+A2) #41 머지·PR4(P3 A1) 구현 완료(브랜치 feat/api-batch-p3-scannow, build green 486·커밋 보류·머지 시 Done). ★P1·P3 전부 완료(A1=마지막 기능 PR). P2(M7)=보류만 잔존)** — 스키마변경=`spec_record.filename` 1건(PR2·재배포 필요). ★중앙영향: M1=배열+헤더(non-breaking)·M2/M4/M6=additive·M5 rationale 가산(additive·ETag 불변)·A1=신규(영향 0). ★매뉴얼 일괄 반영 완료(TW, api-rest-manual.html: D1 삭제·M1~M6·A1·A2·W1, 재배포 c435d9f 실데이터).
  ...
  - 2단계(M7, 고위험·독립 PR) **(상세 설계 완료 → doc/36 / DECISIONS D52, 사용자 UPDATED 스코프 확인 후 dev)** — M7=2 sub-phase:
    - [ ] **M7a(우선·0 스키마)**: filename→specName 도출(미전달=default·하위호환) + `SpecCanonicalProjection`(canonicalJson 포함·**rawDoc 미선택**=oid 회피) + `SpecDiffService`(compute-on-read: 현 active vs 직전 inactive[같은 specName active=false 중 specVersion 최대], 동일성=method+path_template, ADDED/DELETED + **UPDATED=deprecated/version 한정**) + `GET /domains/{host}/spec/changes`(`?specName/from/to`·`updatedScope` 노출·specName 별 changes) + 실 PG 테스트(oid 가드·null 정렬 가드·ADDED/DELETED/deprecated-UPDATED·최초=전ADDED·멀티문서·param-only=미보고)
