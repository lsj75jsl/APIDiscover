...
- [ ] **★스캔 due/워터마크 발산 대응 — 유령(endpoint 0) 도메인 억제·정리 (진행 중, 설계 doc/42·사용자 전건 승인·삭제 hard 채택)** — 유령(스캔해도 자사 endpoint 0)이 39.5k(due 66%)로 큐 잠식 → 밀림 p50 4.4일/max 14일·워터마크 near-now 6.7% 발산(2026-07-14 실측). 원인=봇/스푸핑 Host 가 캐치올 엣지 통해 **status 무관 자동등록** + 봇 재방문이 inactive-after(P3D) 진동. **★실행 순서: A+C 배포(재유입 차단) → 1~2일 관찰 → B/B-2/D 삭제(사용자 승인 후 ops)**.
  ...
  - [ ] B/B-2/D runbook 실행 (★사용자 최종 승인 후 ops 1회·hard DELETE + 백업 D62 선례) — B: 유령 ~28.7k(7일 지속·스캔이력·스펙/사용자설정 제외 안전기준) / B-2: 제외엣지 전용 ~3.1k / D: `last_access_log_at` 백필 ~3.4k(버그 아닌 이력 잔재). ★삭제=트래픽 재발생 시 discovery 자동 재등록=자기치유. FK 순서(domain_hostnames·watermark→domain_config).
