...
- [ ] **REST API 대규모 변경 배치 (삭제·수정·신규)** **(설계 → doc/35 / D50. PR1(D1+M1+M3) #39·PR2(filename+M2+M4+M6) #40·PR3(M5+A2) #41 머지·PR4(P3 A1) 구현 완료(브랜치 feat/api-batch-p3-scannow, build green 486·커밋 보류·머지 시 Done). ★P1·P3 전부 완료(A1=마지막 기능 PR). P2(M7)=보류만 잔존)** — 스키마변경=`spec_record.filename` 1건(PR2·재배포 필요). ★중앙영향: M1=배열+헤더(non-breaking)·M2/M4/M6=additive·M5 rationale 가산(additive·ETag 불변)·A1=신규(영향 0). ★매뉴얼 일괄 반영 완료(TW, api-rest-manual.html: D1 삭제·M1~M6·A1·A2·W1, 재배포 c435d9f 실데이터).
  ...
  - 2단계(M7 **재설계** — 영속 API 인벤토리 + reconcile, 고위험·독립 PR) **(★P1-1~P1-8 구현 완료 → doc/37 / DECISIONS D53. 브랜치 `feat/documented-api-inventory`, build green 493·실패0·skip2, PostgresIntegrationTest 27/27·커밋 보류·머지 시 부모 Done. Zombie 결합·reconcile DELETED 실 PG RED-확인. doc/36/D52[문서버전 diff]=의도 불일치로 supersede)** — ★M7a(PR #44/#45, compute-on-read diff)는 doc/37 §5 롤백 경계로 **제거·이관**(구현됐으나 폐기). 재설계 P1/P2:
    ...
    - [ ] **P1-9 매뉴얼(TW)**: `/apis`·status 의미·deprecated vs DELETED Zombie·/spec/changes 제거 반영.
    - [ ] **P2(후속)**: 인벤토리로 매칭 source 대체(loadActiveCanonical 통합·scan-path 리팩터)·기존 spec param 백필(재업로드 또는 (b)bytea)·풍부 param diff(type 상세·breaking)·도메인-merged 뷰·inactive prune. (doc/37 §10)
