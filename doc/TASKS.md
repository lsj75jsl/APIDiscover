...
- [ ] **REST API 대규모 변경 배치 (삭제·수정·신규)** **(설계 → doc/35 / D50. PR1(D1+M1+M3) 머지 #39. PR2(filename+M2+M4+M6) 구현 완료(브랜치 feat/api-batch-p1-specmeta, build green 472·커밋 보류·머지 시 Done). 나머지 M5·A2·P3·P2 후속)** — 스키마변경=`spec_record.filename` 1건(PR2·재배포 필요). ★중앙영향: M1=배열+헤더(non-breaking)·M2/M4/M6=additive·M5 rationale 가산(후속).
