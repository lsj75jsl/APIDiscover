...
- [ ] **무접속 도메인 상태 관리 — 7일 무요청 비활성·경로 제외·재활성 (설계 확정 doc/43·D82·2026-07-20·구현 전)** — 원인 규명(2026-07-20 실측): 워터마크 롱테일 발산은 **window 상한(attempted-but-lagging 0)·라운드로빈 기아(selectable 미시도 0)가 아니라 inactive-after=P3D 게이트 스킵**(굶는 enabled 8,201=100% lastSeenAt>3d). 7일 미스캔 도메인 29,082(enabled 8,201=게이트분·disabled 20,881=07-02 유령잔존). **사용자 확정: 임계 기본 P7D·설정 override / status = 안B 영속 enum / 수동 스캔은 주기 스캔 대상 승격.**
  - [ ] 【필수】discovery 경로 제외 — `buildPattern()` 인덱스8→`<uri>` 명명 + `discovery.excluded-paths`(기본 `.cloudbric/pron|afc`·빈=무회귀) + `buildLogQL` `\| uri !~` 라벨필터(status 필터와 동일 안전패턴). 두 경로만 받는 도메인 관측 0→lastSeenAt 정체→INACTIVE.
  - [ ] 【설정】`scan.inactive-after` 기본값 P3D→**P7D**(설정 override 유지).
  - [ ] 【안B】`DomainConfig` `activity_status`(enum·기본 ACTIVE·인덱스)+`activity_status_changed_at`(nullable) ddl-auto ADD.
  - [ ] 【안B】전이 3경로 — discovery 틱 종료 sweep(ACTIVE→INACTIVE, lastSeenAt<cutoff bulk UPDATE) / `DomainUpserter.upsert` 실요청 재관측→ACTIVE / `ScanController` scan-now·scan 수동→ACTIVE(직접 UPDATE·승격).
  - [ ] 【안B】스캔 술어 교체 — findDueForScan·findDueWithNewTraffic·findDueWithoutNewTraffic 의 lastSeenAt staleCutoff → `activity_status='ACTIVE'`, ScanSelector staleCutoff/EPOCH 제거. status 노출 DTO(activityStatus+changedAt).
  - [ ] 테스트 — buildLogQL 스냅샷(uri 무회귀)·경로제외 lastSeenAt/status 미갱신·sweep P7D 경계·재관측/수동 flip·술어 ACTIVE만 선택·기본 ACTIVE 무회귀·실 PG(ddl-auto·bulk UPDATE·인덱스 D75).
  - [ ] 【검증·부하안전】uri 필터 부착 쿼리 Loki 지연/에러 사전 실측(짧은 창·hostname 스코프, 광역스캔 금지) + 경로 정규식 실 uri 표기 확인.
