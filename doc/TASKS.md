...
- [ ] **8.3 로그변수 소비 — 응답 Content-Type + 요청측 API 신호(accept/xhr/origin/auth)** **(구현·매뉴얼·재배포 완료 — main `05c17aa`(PR #73)·매뉴얼 `13706bb`·이미지 `fbe3a9c` .197 배포 DORMANT 무회귀[신규 4컬럼 ddl-auto ADD·3,046,006행 false 검증·롤백 prev-pre-d79]. 활성화만 잔여: 운영 로그포맷 표준화 선행 → C안 규약 명세 작성 중·nginx/엣지통일 운영 조율 blocked)** — nginx 로그 8.3 append 필드 소비. `$server_protocol`·`$upstream_addr` 제외. ACRM 은 이미 구현(설정만). 신규 인덱스 기본 -1 DORMANT·양성 가산 → 무회귀. 상세=doc/40. ★활성화 제약(dev 실측 2026-07-14): 엣지 로그 필드 수 이질(18/19/24) → '끝 append'가 엣지마다 다른 절대 인덱스 → **대상 엣지 24필드 통일/한정 필수**(단일 인덱스 파싱 전제).
  ...
  - [ ] log_format 규약 명세(운영 전달, C안) — **작성 중(dev, doc/41 예정)**: 코어 24필드 인덱스 정의·8.3 append 규약(24~30)·★엣지 24필드 통일 요구(18/19/24 이질)·nginx log_format 예시·활성화 시 application.yml 인덱스 세팅 절차.
