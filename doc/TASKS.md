...
- [x] **응답 배열 전역 `{count, items}` 래핑 (D86)** **(구현·테스트·매뉴얼 완료 — build green 575·커밋/배포 진행. 사용자 요청)** — 모든 API JSON 의 모든 배열(필드값·최상위·중첩)을 `{count, items}` 로 감싸 파서가 `.count`·`.items` 로 일관 접근. `ArrayCountJson`+`ArrayCountResponseAdvice`(전역 ResponseBodyAdvice)·`/scan-result/detail` 은 inlineBasis 직접 래핑. ★breaking(배열→객체). 통합/컨트롤러/inlineBasis 테스트 `.items`·매뉴얼 전 예시+전역 콜아웃 반영. → D86.
- [x] **/discovery 요약·상세 분리 + Finding classification 인라인 (D85)** **(구현·테스트·매뉴얼 완료 — build green 575·커밋/배포 보류. 사용자 요청)** — ① `GET /discovery` = 요약(`DiscoverySummaryView` host·summary·apis, `/scan-result` 와 동형·창 무관 누적), `GET /discovery/detail` = 기존 full CombinedDiscovery. ② `Finding.classification()` @JsonProperty → 모든 findings[] 가 유형(shadow/zombie…) 인라인 직렬화(근본: 그동안 타입 판별자 없어 미직렬화). `ApiLists.label()` 공용. ★`/discovery` breaking(요약 축소)·report_json additive. 매뉴얼(§2.3 분리·요약키표·detail classification·요약표·TOC) 반영. → doc/DECISIONS D85.
