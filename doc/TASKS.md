...
- [ ] **(후속·조사) `DiscoveredEndpoint.signature` 원천 발산 근본 정리** — signature 가 (method, 스캔host, 최종 template)와 어긋나는 경로 존재(InventoryBuilder 가 T1 승격 전 template·파싱 host 로 signature 생성). upsert dedup 은 제약 튜플 키로 우회 완료. **잔여 P3(무크래시·조용한 정확도)**: classifier recency 가 `priorFirstSeen`(signatureOf=제약튜플 키)에 `d.signature()`(발산값)로 lookup → 발산 endpoint recency miss → Zombie severity 과소. 권장 근본=signature 를 최종 resolved 값으로 생성하거나 `d.signature()` 소비처 전부 `identityKey` 통일(upsert·recency 일괄). 별도 설계 후속.
