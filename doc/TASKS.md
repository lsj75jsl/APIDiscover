...
- [ ] **(후속·범위 밖) signature 원천 발산 + foreign-host 누수** — (a) `DiscoveredEndpoint.signature` 를 InventoryBuilder 가 T1 승격 전 template·파싱 host 로 생성(소비처 0이라 무해하나 근본=resolved 생성/필드 제거, 테스트 얽힘). (b) **foreign-host 누수**: `LokiQueryBuilder` `|= domain` substring 필터가 referer/URL 의 다른 Host 라인도 매칭 → `d.host()≠스캔도메인` endpoint 가 스캔도메인 하에 영속. upsert·recency 는 스캔host 키잉으로 정합화했으나 누수 자체는 소스 필터(exact host)·InventoryBuilder host 필터로 별도 해결.
