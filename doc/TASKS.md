...
- [ ] **8.3 로그변수 소비 — 응답 Content-Type + 요청측 API 신호(accept/xhr/origin/auth)** **(구현·매뉴얼·재배포 완료 — main `05c17aa`(PR #73)·매뉴얼 `13706bb`·이미지 `fbe3a9c` .197 배포 DORMANT 무회귀[신규 4컬럼 ddl-auto ADD·3,046,006행 false 검증·롤백 prev-pre-d79]. 활성화만 잔여: 운영 로그포맷 표준화 선행 → C안 규약 명세 작성 중·nginx/엣지통일 운영 조율 blocked)** — nginx 로그 8.3 append 필드 소비. `$server_protocol`·`$upstream_addr` 제외. ACRM 은 이미 구현(설정만). 신규 인덱스 기본 -1 DORMANT·양성 가산 → 무회귀. 상세=doc/40. ★활성화 제약(dev 실측 2026-07-14): 엣지 로그 필드 수 이질(18/19/24) → '끝 append'가 엣지마다 다른 절대 인덱스 → **대상 엣지 24필드 통일/한정 필수**(단일 인덱스 파싱 전제).
  - [x] §5 시뮬레이션 — **완료(2026-07-13, `sample/log_signal_promotion_sim.py`, 운영 PG read-only 전수, 결과 doc/40 §5.1).** ApiScorer(MIDDLE) 파이썬 정확 포팅(13신호, pathHint만 미재현). 게이트 분포: ADMIT 33,527(1.14%)·DROP_LOW_SCORE 1.78M(60%)·API_CANDIDATE 0건. **격하 0건 재확인**(양성가산·단조). **과승격 상한**: 단일신호 최대 ≤31,391(전체 1.06%, 89.5%가 API 구조 보유=옳은 승격)·2신호 ≤422k·전신호=LOW 전체. **결론: §3 가중치 유지 권장(잘 조정됨)·발화조건 다수결(`count*2≥hits`) 확정 권장.**
  - [x] §6 코어 — **완료(PR 대기, 커밋 9a17350).** ParseProperties(5 신규 인덱스 -1·단일 canonical 생성자+`acrmOnly`/`defaults` 팩터리 — ★복수 생성자면 @ConfigurationProperties 바인딩 실패)·application.yml(parse 블록 주석)·LogLineParser(`readOptional` nullable read)·ParsedRequest(5 필드·14-arg 하위호환)·Acc(CT 2xx-only 누적·정규화·요청 4신호 presence 다수결)·DiscoveredEndpoint(4 불리언·11-arg 하위호환)·Record(4 컬럼 ddl-auto ADD default false)·upsert 쓰기·serve-time 복원(CombinedDiscoveryService)·InventoryBuilder 배선.
  ...
  - [x] log_format 규약 명세(운영 전달, C안) — **완료(dev, `doc/41-nginx-log-format-spec.md`·PR #74 `0002e4c`·D80)**: 코어 24필드 인덱스 정의·8.3 append 규약(24~30)·★엣지 24필드 통일 요구(18/19/24 이질, 실측 AAJ14/PAK21/PARV2/PLDI1)·nginx log_format 지시문+auth_scheme map·활성화 시 인덱스 세팅 절차.
  - [ ] 활성화 + 활성 단계 검증 — **운영 조율 blocked**: nginx log_format 8.3 append + 엣지 24필드 통일(운영 측) 완료 후 → application.yml/adc.yaml 인덱스 24~30 세팅 → **전/후 스냅샷 diff 로 kind 재분류·격하 실측**(CT kind-flip 은 사전 시뮬 불가, doc/40 §4.3 잔여 위험). ★**활성화 대상 선별 완료**(dev 전수조사 PR #75 `ca53868`·매뉴얼 §8.5): 대상 213엣지(제외 227) 중 **24필드 158=즉시 후보**·나머지 55(23=46·18=5·25=1·27=2·라인없음=1)는 24통일 또는 대상 제외. ★엣지 24필드 통일 근거 = dev 전수조사(2026-07-14, 대상 213엣지 중 24필드=158 즉시후보·나머지 55 통일/제외, `sample/edge_field_count_result.txt` main ca53868) → 매뉴얼 **§8.5**(활성화 전 수정 사항·개수별 엣지명 전수표) 반영 완료(TW, 2026-07-14).
...
- [x] **응답 배열 전역 `{count, items}` 래핑 (D86)** **(완료·배포 — PR #79 머지 `main 944bd0f`·이미지 `217429868306` .197 배포·health UP·롤백 `prev-d85`·실측 검증. build green 575. 사용자 요청)** — 모든 API JSON 의 모든 배열(필드값·최상위·중첩)을 `{count, items}` 로 감싸 파서가 `.count`·`.items` 로 일관 접근. `ArrayCountJson`+`ArrayCountResponseAdvice`(전역 ResponseBodyAdvice)·`/scan-result/detail` 은 inlineBasis 직접 래핑. ★breaking(배열→객체). 통합/컨트롤러/inlineBasis 테스트 `.items`·매뉴얼 전 예시+전역 콜아웃 반영. → D86.
- [x] **/discovery 요약·상세 분리 + Finding classification 인라인 (D85)** **(완료·배포 — PR #79 머지 `main 944bd0f`·이미지 `217429868306` .197 배포·실측 검증. D86 과 함께. build green 575. 사용자 요청)** — ① `GET /discovery` = 요약(`DiscoverySummaryView` host·summary·apis, `/scan-result` 와 동형·창 무관 누적), `GET /discovery/detail` = 기존 full CombinedDiscovery. ② `Finding.classification()` @JsonProperty → 모든 findings[] 가 유형(shadow/zombie…) 인라인 직렬화(근본: 그동안 타입 판별자 없어 미직렬화). `ApiLists.label()` 공용. ★`/discovery` breaking(요약 축소)·report_json additive. 매뉴얼(§2.3 분리·요약키표·detail classification·요약표·TOC) 반영. → doc/DECISIONS D85.
- [x] **scan-status 유형별 API 목록 + `/result` reason 재배치 + 엔드포인트 개명 (D84)** **(완료·배포 — PR #77 머지 `main dafbb69`·매뉴얼 반영·이미지 `100606f00f38` .197 배포·health UP·롤백 `prev-d84`·실측 검증. build green 573. 사용자 요청)** — ① `GET /scan-result`(구 `/scan-status`)에 `apis{discovered,active,shadow,zombie,unused}` 가산(각 원소 `"GET [https://host/path]"`). per-scan report_json finding 을 serve-time `forHost` 판단근거로 분류(=summary 와 동일 집합·개수 일관). classification 은 report_json 미저장(타입 소거)이라 detail 인라인과 동일 매칭 경로 재사용(`rationaleByKey` 공용 추출). report_json 없으면 빈 목록·forHost 미호출(경량 유지). scheme 미저장→https 고정. ② `GET /scan-result/detail`(구 `/result`) finding 필드 순서 변경 — `reason` 을 `classification` 뒤로 재배치(inlineBasis 에서 remove→재부착). ③ **엔드포인트 개명**: `/scan-status`→`/scan-result`, `/result`→`/scan-result/detail`(@GetMapping 2곳+통합테스트 4곳, 메서드명 유지). ★**breaking(경로 변경)**+중앙/매뉴얼(TW) 반영 필요. ★scan-result 가 forHost 호출로 무거워짐(설계상 경량 메타였음)=사용자 명시 요청 트레이드오프.
...
### P3. 운영/인프라 (자체 운영)
- [ ] **★스캔 due/워터마크 발산 대응 — 유령(endpoint 0) 도메인 억제·정리 (진행 중, 설계 doc/42·사용자 전건 승인·삭제 hard 채택)** — 유령(스캔해도 자사 endpoint 0)이 39.5k(due 66%)로 큐 잠식 → 밀림 p50 4.4일/max 14일·워터마크 near-now 6.7% 발산(2026-07-14 실측). 원인=봇/스푸핑 Host 가 캐치올 엣지 통해 **status 무관 자동등록** + 봇 재방문이 inactive-after(P3D) 진동. **★실행 순서: A+C 배포(재유입 차단) → 1~2일 관찰 → B/B-2/D 삭제(사용자 승인 후 ops)**.
  - [x] A+C 구현·배포 — **완료(머지 `f089a40` PR #76·이미지 `a171edf` .197 배포·health UP·롤백 `prev-pre-d81`, 2026-07-14)**. A: `NEW-PAJ*` 제외(excludedHostnames 25개 발효·excludedEdge 17,096). C(근본): `discovery.probe-statuses [404,470]` 발효(buildLogQL `status!~404|470`·discovery vector=28,226 정상 반환). ★배포 후 기준선: enabled 67,308·due 53,256·near-now(30m) 7.1%·ghost 39,620·cleanup-eligible 28,722(설계 실측 일치). 메모리 `ghost-suppression-ac-deploy-baseline` 원점 기록.
  - [ ] **★★C 배포 후 꼭 확인할 항목 (blocklist 조정 판단 — 사용자 지시)** — `status_dist_json` 이 **4xx 통합**이라 봇이 정확히 404/470 인지 DB로 미확증. C 배포 후 **"신규 유령 유입/일"**(발산 진단 쿼리 재실행: enabled·discovered_endpoint 0·discovered_at 최근) 로 blocklist `[404,470]` 효과 검증. **봇 유입 지속 시 status 목록 확대(예: `403` 추가) 검토** — ★단 403 추가 시 권한 걸린 실 API(403) 손실 가능(트레이드오프). 근거(실측 2026-07-14): 성공(2xx) 없는 endpoint 36% = 대부분 봇 프로브(`.env`·`/*`·`/-`·와일드카드)·정적 3xx 리다이렉트, **401-only 실 API 는 희소** → blocklist 유지·allowlist(2xx만) 미채택(3xx/5xx/401/403 과잉배제 회피). doc/19(404 저격·401/403 보존)와 일관.
  - [x] B/B-2/D 실행 — **완료(2026-07-15·★백업 없이·사용자 결정)**. hard DELETE **31,985 도메인**(ghost 29,361 + excluded-only 2,624·안전기준 통과·unsafe 0) + FK 연계(domain_hostnames 41,294·watermark 31,985·scan_result 31,236·discovered_endpoint 25,867) + D 백필 last_access_log_at 2,240. ★앱 가동 중 데드락→adc-app 잠깐 stop(2분)·무경합 원자 트랜잭션·dry-run/temp셋/pre-post guard. **결과: enabled 67,347→35,362·due 53,256→20,264(−62%)·near-now 7.1%→9.0%·orphan 0(FK 정합)**. 잔여 유령 10,243=7일 미만(제외분·지속 시 후속). 자기치유=트래픽 재개 시 재등록.
  - [ ] 사후 모니터링 — due·밀림 p50/p90·near-now 비율, 삭제 도메인 재관측(자기치유) 점검.
- [ ] **무접속 도메인 상태 관리 — 7일 무요청 비활성·경로 제외·재활성 (구현·배포 완료 doc/43·D82·main `5bced89`·이미지 `4d876686b814` .197 배포, 2026-07-20. 잔여=사후 관찰만)** — 원인: 워터마크 롱테일 발산은 window 상한·라운드로빈 기아 아닌 inactive-after=P3D 게이트 스킵(굶는 enabled 8,201=100% lastSeenAt>3d). **사용자 확정: 임계 P7D·status 안B 영속 enum·수동 스캔 승격.** 566 테스트 green·롤백 `prev-d82`(a171edf).
  - [x] 【필수】discovery 경로 제외 — `buildPattern()` 인덱스8→`<uri>`(F_REQUEST_URI 공개) + `discovery.excluded-paths`(기본 `/.cloudbric/pron|afc`) + `buildLogQL` `\| uri !~ 백틱정규식`(★이중따옴표는 `\.` 이스케이프 위반→Loki 400, 백틱 raw 확정. 운영 2분창 status=200 실검증).
  - [x] 【설정】`scan.inactive-after` 기본 P3D→**P7D**(설정 override 유지).
  - [x] 【안B】`activity_status`(enum·`columnDefinition default 'ACTIVE'`=ddl-auto ADD 무회귀·기존 57,566행 ACTIVE)+`activity_status_changed_at`·복합 인덱스`(activity_status,next_scan_due_at)`.
  - [x] 【안B】전이 3경로 — discovery 틱 종료 sweep(`deactivateStale`) / `DomainUpserter` 실요청 재관측→ACTIVE / `ScanController` scan-now·scan `markActive`.
  - [x] 【안B】스캔 술어 교체 — findDue* 3쿼리 lastSeenAt staleCutoff → `activity_status='ACTIVE'`, ScanSelector staleCutoff/EPOCH 제거. DomainDetailView activityStatus 노출.
  - [x] 테스트 566 green — buildLogQL uri 필터·경로제외·sweep/markActive/재관측 flip·enum 게이트·기본 ACTIVE 무회귀·실 PG(ddl-auto·bulk UPDATE·인덱스).
  - [x] 【검증】운영 Loki 2분창 uri 필터 status=200 사전 실측 + 배포 후 첫 discovery vector=16,986·Loki 4xx/5xx 0·deactivated=37,190·configprops excludedPaths 2개 확인.
  - [x] 사후 관찰 — 발산 해소 확인(스캔대상 100% 24h·중앙 ~30분·3일+ 0). ACTIVE 20.4k→10.7k 수렴(프로브·`.cloudbric`-only 이탈).
  - [x] **후속 1단계: 자사 서비스 아닌 도메인 제외** (main `d426b75`·이미지 `bbb3d737a508` 배포·롤백 `prev-cbric`) — `discovery.excluded-domain-suffixes` `[cbricdns.com]` 접미 + IPv4 리터럴 제외. `excludedDomain=60`/사이클 동작. cbricdns ACTIVE ~1,795·IP 544 가 P7D 후 이탈 → scannable ~8.9k 수렴 예상(점진).
  - [x] 【정리 완료】ddl-auto WARN — `@Column(columnDefinition)` → `nullable=false + @ColumnDefault("'ACTIVE'")` 분리. 재배포 후 재기동 ALTER 경고 **0건** 확인(main `c07891e`).
  - [x] **scan 정합** (main `c07891e`·이미지 `7632f074c9f0` 배포·롤백 `prev-scanfix`) — `DiscoveryJobService.analyze()` 가 `discovery.excluded-paths` 요청을 인벤토리에서도 제외(discovery 와 일관·rawPath 접두 매칭). 신규 discovered_endpoint 에 `.cloudbric` 미기록. 569 테스트 green.
  - [x] **기존 `.cloudbric` endpoint 정리** — 단일테이블 DELETE(FK 없음·앱 가동 중 안전, 정합 배포 후 경합 없음) **5,430행 삭제·잔존 0**(2026-07-22, 사용자 승인). 사전 5,430(5,037 host)→사후 0.
  - [x] **2단계 www+bare 병합(분석)** — ACTIVE 10,760 FQDN → www 병합 시 **8,959**(www 2,721 중 bare 짝 1,801 병합). 서비스 근사 eTLD+1 ~3,714(실서비스 ~6k 중 활성분, 나머지 조용→INACTIVE). 시스템 레벨 dedup 은 미실행(분석만).
  - [x] **3단계 endpoint 0 드릴다운** — 1,837개(74% 신규 유입) = 봇/foreign-host. 최상위 유입 엣지 `new-PAJ11`(947도메인)이 **실서비스 501+유령 439 혼합** → 엣지 제외 불가. (부수: EdgeExclusions 대소문자 구분으로 new-PAJ11 이 NEW-PAJ* 회피, 단 실서비스 보유라 제외 금지.)
  - [x] **endpoint-yield 게이트(D83)** (main `c9e93b5`·이미지 `a2b31a1c8c4b` 배포·롤백 `prev-ghostgate`) — `ghost_suppressed` boolean + `scan.ghost-after`(P7D·0=off). discoveredAt<cutoff+스캔이력+0-endpoint+무설정(interval/basepath/spec/documented) → 억제. 수동 스캔=해제(가역). 스캔대상=enabled AND ACTIVE AND NOT ghostSuppressed. 배포 첫 게이트 537 억제·scannable 10,729→10,153·실서비스 보존·571 테스트 green·DDL 경고 0.
  - [ ] 후속(대기): full eTLD+1(PSL·SaaS 카운팅 규칙) 서비스 수 정밀화 — 근사 실서비스 ~3,160(활성)·등록 ~6k 중 ~2.8k 조용. / ghost 게이트 수렴 관찰(다음 며칠 scannable 추가 하락).
- [x] **무접속 도메인 자동스캔 제외 (마지막 실 로그 > 30일)** **(구현 완료 — build green 505·실 PG OK·RED-확인·커밋 보류·머지 시 Done. D55→D57 재설계, 사용자 확정)** — `scan.inactive-after`(기본 `P30D`·0/null=비활성) + `ScanSelector.findDueForScan` 제외. ★기준=**Loki 실 access log 시각**(`domain_config.last_access_log_at`=max discovered_endpoint.last_seen[time_iso8601], analyze 의 `touchLastAccessLogAt` never-decrease) — `last_seen_at`(discovery 관측시각) 아님(D57 정정). scan-now(명시)는 항상 스캔·자동스캔만 제외. per-domain Loki 쿼리 없음(스캔이 이미 읽는 값 재사용). null(미스캔)=제외 안 함. EPOCH 센티넬(비활성). 테스트: ScanSelectorTest·PostgresIntegrationTest(lastAccessLogAt 게이트).
- [x] **정적분류 규칙(확장자·토큰) DB 외부화 + reload** **(구현 완료 — build green 505·실 PG CRUD/reload OK·커밋 보류·머지 시 Done. D57, 사용자 요청)** — 소스 하드코드 → DB. `static_classify_rule`(kind·value·ddl-auto ADD) + `StaticClassifyRules`(seed 기본값·DB→`EndpointKindClassifier.applyRules`·add/remove 즉시 재적용) + REST `/api/v1/config/static-classify`(GET 목록·POST 추가·DELETE 삭제·POST /reload). EndpointKindClassifier 배열→volatile Set(런타임 교체)+기본상수. ApiScorer new 생성 대응=정적 volatile·단위테스트 @BeforeEach 리셋. 매뉴얼(TW)=후속.
...
- [x] **스펙 업로드→reconcile E2E 실검증**: messenger-api.everhrm.com 발견 API 를 OpenAPI 로 문서화→업로드→scan-now. 4분류(Active 4·Zombie 1·Unused 2·Shadow 81) 설계대로. 실 도메인이라 검증 후 원복.
- [x] **D70 Swagger 2.0 지원**: `OpenAPIV3Parser`→`OpenAPIParser`(2.0 v2-converter 자동 변환)·`toOrigin()` protocol-relative(`//host/base`) 처리·`SpecController` 파싱실패 400 매핑. build green 536(신규 2)·RED-확인. OpenAPI 3.x·Postman·CSV 무회귀.
...
- [x] **D67 배치 10→20 + 기본값 승격**: 실측 쿼리 −13.7%(캡 6000 이내 복귀). 검증 설정(D58~D67) 전부 application.yml 기본값 승격 — 신규 서버 env 없이 동일 동작(adc.yaml=DB 접속 등 고유값만). 매뉴얼 설정표 재편. **1h 실측(18:13 KST)**: 4,933쿼리/h(<6000)·deferred/FAILED 0·saves/h +70%(10,072)·활성 fresh 63%·지연 43분.
- [ ] (후속·선택) 배치 캡 30 단계 상향 — **1h 실측 결과 불요 판단(페이스 여유 ~17%), 보류**. 주간 피크 페이스가 6000 재근접 시 검토(+4.2%p). fill=1(39%)·페이지네이션(24%)은 캡 무관 — 추가 절감은 교차-엣지 배칭 등 설계 변경 필요.
