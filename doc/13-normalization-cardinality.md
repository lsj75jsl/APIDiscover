...
### 공유/설정
- [x] `config/NormalizationProperties`(@ConfigurationProperties `apidiscover.normalization`: maxTemplatesPerHost=5000,
...
- [x] `model/DroppedByLimit(int templates, int params)` + `@JsonProperty total()`.

### T1
- [x] `normalize/CardinalityNormalizer` — 통계 {var} 승격(클러스터·ratio≥0.3·distinct≥20·수렴≥0.7) + 상한(host/param)
      → 승격·캡 적용 결과 + `DroppedByLimit` 반환.
- [x] `InventoryBuilder` 에 패스 3·4 통합(승격 후 상한).

### T2
- [x] `model/ParsedRequest`: `queryKeys`→`queryParams: List<QueryParamObs>` (+`QueryParamObs(name, ValueLenBucket)`, `enum ValueLenBucket`).
- [x] `parse/LogLineParser`: query 값 길이 버킷화(값 폐기) → queryParams 채움. 기존 queryKeys 사용처/테스트 갱신(grep 확인).
- [x] `model/ParamCandidates(query: List<QueryParam>, path: List<PathParam>)` + `QueryParam(name,count,lenBuckets,sensitive)` + `PathParam(position,token)`.
- [x] `model/DiscoveredEndpoint`: `ParamCandidates params` 필드 추가(가산).
- [x] `normalize/ParamCandidateExtractor` — Acc query obs + 템플릿 변수세그먼트 → ParamCandidates, per-endpoint param 상한(→DroppedByLimit.params), sensitive 적용.

### T3
- [x] `config/SensitiveKeyProperties`(`apidiscover.sensitive-keys.{names,patterns}` 기본값 내장).
- [x] `normalize/SensitiveKeyMatcher`(@Component) — names+regex 대소문자 무시 `isSensitive(key)`. 기본 정책 이름 보존+flag+버킷 억제.

### 보고/배선
- [x] `model/Finding.Shadow` 에 `ParamCandidates params` 추가(Classifier 가 d.params() 전달).
- [x] `model/DiscoveryReport` 에 `DroppedByLimit droppedByLimit` top-level 추가, `report/ReportBuilder.build` 파라미터 추가.
- [x] `normalize/InventoryBuilder`: `InventoryResult(endpoints, DroppedByLimit) buildWithLimits(...)` 추가, `build→List` 위임(하위호환).
- [x] `batch/DiscoveryJobService.analyze`: buildWithLimits 전환 + droppedByLimit 를 ReportBuilder 전달 + ETag 입력에 추가.

### 테스트
- [x] `CardinalityNormalizer` — 고카디널리티(≥20 distinct slug) 승격·재병합, 소표본/저비율 미승격(무회귀), 수렴 가드(소형 enum 미승격).
- [x] 상한 — >5000 템플릿/>50 param 초과분 drop(hits 낮은 순) + DroppedByLimit 카운트.
- [x] `ParamCandidateExtractor` — query name+presence+버킷, path 변수세그먼트 후보.
- [x] `SensitiveKeyMatcher` — 기본 키/정규식 flag, 버킷 억제, 비민감 무영향.
- [x] `LogLineParser` — queryParams name+버킷 정확, **값 미저장** 검증.
- [x] 하위호환/무회귀 — 기존 inventory 템플릿 동일(미승격), DroppedByLimit=(0,0), 리포트 가산적, ETag 가 droppedByLimit 반영.
