...
> [36-spec-multidoc-status](36-spec-multidoc-status.md)(M7a 문서버전 diff 모델)를 대체. 업로드마다 문서(specName)별로 **영속 API 인벤토리**(`documented_api`)를 reconcile — 등록/변경/삭제(ADDED/UPDATED/DELETED)를 현재 상태 속성으로 관리하고 파라미터를 보유하며 DELETED 를 Zombie 입력으로 쓴다. 근거 [26](26-multi-spec-merge.md)(멀티스펙·SpecStore)·[03](03-spec-formats-and-canonical-model.md)(canonical)·[28](28-testcontainers-pg-integration.md) §10(rawDoc oid), 결정 **DECISIONS D53**(D52 supersede).
> **구현 완료(P1, PR #46 머지)** — §10 참조. 후속 P2 는 [38-spec-inventory-p2](38-spec-inventory-p2.md).

**구현 위치**

| 대상 | 소스 |
|---|---|
| 영속 인벤토리 | `domain/DocumentedApiRecord`(`documented_api`, unique host+specName+method+path_template) + `DocumentedApiRepository` |
| 파라미터 | `model/SpecParam(name, in, required, type)` + `ParamIn` + `CanonicalEndpoint.params` |
| reconcile | `spec/ApiInventoryService.reconcile()`(`SpecStore.upload` @Transactional 내, specName 단위·삭제 격리) |
| 노출 | `api/ApiInventoryController`(`GET /api/v1/domains/{host}/spec/apis`) + `spec/DocumentedApiView` |
| DELETED→Zombie | `classify/Classifier`(1st pass, host 의 DELETED 인벤토리 키 → deleted-from-spec Zombie) |
...
```mermaid
flowchart TD
    U["upload(host, specName, parsed)"] --> E["existing = documented_api WHERE host, specName"]
    E --> L{"각 parsed 엔드포인트 (method+pathTemplate)"}
    L -->|"미등록"| ADD["INSERT status=ACTIVE, lastChange=ADDED"]
    L -->|"등록·DELETED"| RES["resurrect → ACTIVE, ADDED"]
    L -->|"등록·param/deprecated/version 변경"| UPD["UPDATED (params 갱신)"]
    L -->|"등록·동일"| UNC["UNCHANGED"]
    E --> D{"existing ACTIVE 중 parsed 에 없는 키"}
    D --> DEL["status=DELETED (이 specName 한정 — 다른 문서 불변·union)"]
    DEL --> Z["DELETED ∩ 관측 트래픽 → Zombie(deleted-from-spec, §6)"]
```
...
- **신규 `GET /api/v1/domains/{host}/spec/apis`**(인벤토리 목록). 필터 `?specName=`·`?status=ACTIVE|DELETED`·`?method=`. host 정규화 null→400. **oid 무접근**(documented_api projection/엔티티는 oid 없음).
...
| `integration/PostgresIntegrationTest.java` | **수정** — `specChanges*` 4건 제거 | `reuploadSameFilenameViaHttpDoesNotHit500` 의 /spec/changes 단언을 **`GET /spec/apis` 단언으로 이관**(재업로드 200·reconcile 결과 검증 유지) |
| `doc/36` | **supersede 표기**(본 문서가 대체) | 이력 보존(삭제 아님) |
| 매뉴얼 `api-rest-manual.html §2.4` | **수정**(TW) — /spec/changes 절 제거 → `/spec/apis` 절로 교체 | dev 머지 후 TW 동기 |
...
### 6.6 P1 vs P2 권고
- **데이터 모델 + reconcile + `GET /spec/apis`(DELETED 노출 포함) = P1**(필수). DELETED 가 즉시 조회 가능(노출).
...
- `GET /spec/apis` 신규 = **additive**(영향 0).
- `GET /spec/changes` 제거 — M7a 가 막 머지돼 **중앙 미통합 전제**(의도 불일치를 조기 정정). 중앙팀에 의존 부재 확인 후 제거(있으면 `/spec/apis` 로 전환 조율).
...
- ① **문서 A 업로드** → `documented_api` 에 A 의 API + params(status ACTIVE·lastChange ADDED). `GET /spec/apis` 단언.
...
- ⑤ **oid 안전** — reconcile + `GET /spec/apis` 가 rawDoc(엔티티) 미접근으로 비-tx auto-commit 200(§7(a) 적용 시 rawDoc 부재라 구조적 안전·H2 미재현→실 PG). 회귀 시 RED.
- ⑥ **h2-pg null 정렬 가드(D48)** — `/spec/apis` ORDER BY(specName nullable·레거시 null) `nulls first`/`coalesce(spec_name,'default')` 미명시 회귀 시 실 PG 순서 발산.
...
**P1 (재설계 본체 — 롤백 + 영속 인벤토리)** — 구현 완료. M7a diff 롤백(§5), `DocumentedApiRecord`+repository, `SpecParam`/`ParamIn`+`CanonicalEndpoint.params`(null-safe), 3 파서 param 추출(OpenAPI getParameters/getRequestBody·Postman url.query/path/body·CSV `params` 컬럼), reconcile-on-upload(§3·삭제 격리), `GET /spec/apis`, DELETED→Zombie 결합(§6.5), rawDoc 삭제(§7), 실 PG 테스트(§9)·매뉴얼 반영.

**P2 (후속) — ★상세 설계 = [38-spec-inventory-p2](38-spec-inventory-p2.md) / DECISIONS D54**.
...
- **P2-3 풍부 param diff + breaking 판정**(doc/38 §3) — `applyUpdate` 에서 delta+breaking 계산·저장(신규 컬럼 2개 가산)·`/spec/apis` 노출. 제품가치 최고.
...
- **무회귀** — 신규 테이블 ADD(무손실)·`/spec/apis` additive·`CanonicalEndpoint` params 가산(매칭키 불변)·Zombie 결합 가산(스캔 source 불변)·rawDoc 삭제는 dead 컬럼 제거(read 0). 스캔/분류 매칭 경로 불변.
